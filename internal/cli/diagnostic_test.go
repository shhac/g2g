...
func TestDebugIsPersistentStderrOnlyAndDoesNotChangeLinkMutation(t *testing.T) {
	for _, args := range [][]string{{"--debug", "github", "link", "--branch", "beta"}, {"github", "link", "--debug", "--branch", "beta"}} {
		...
		for _, expected := range []string{"event=operation.start", "operation=\"github link\"", "target_source=\"--branch\"", "event=discovery.target", "event=discovery.trunk", "event=github.native_stack_membership", "observation=\"per_pull_request\"", "event=link.plan", "decision=\"ready\""} {
...
func TestNormalLinkLeavesStderrQuiet(t *testing.T) {
	...
	command.SetArgs([]string{"github", "link"})
	if err := command.Execute(); err != nil {
		t.Fatal(err)
	}
