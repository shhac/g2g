...
func newGitHubStatus(service link.Service, completions stack.Completions, presentation Presentation) *cobra.Command {
	var selection stackOptions
	cmd := &cobra.Command{Use: "status", Short: "Inspect a stack, its pull requests, and native GitHub membership (read-only)", Args: cobra.NoArgs}
	cmd.RunE = func(cmd *cobra.Command, _ []string) error {
		...
		if err != nil {
			...
			// stacked yet — while the offline status rendered the very same fact and exited
		...
		return writeGitHubStatus(cmd.OutOrStdout(), plan, presentation)
	}
