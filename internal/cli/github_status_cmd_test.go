...
func TestStatusRendersCompactAlignedAndBlockedPath(t *testing.T) {
	...
	if err := writeGitHubStatus(&out, plan, Presentation{}); err != nil {
		t.Fatal(err)
	}
...
func TestStatusAdviceStaysOneLineForMachines(t *testing.T) {
	...
	var out bytes.Buffer
	if err := writeGitHubStatus(&out, plan, Presentation{Format: formatPorcelain}); err != nil {
		t.Fatal(err)
	}
...
func TestBaseAndHeadAreSaidSeparatelyAndColouredSeparately(t *testing.T) {
	...
	var out bytes.Buffer
	if err := writeGitHubStatus(&out, plan, Presentation{Color: true}); err != nil {
		t.Fatal(err)
	}
