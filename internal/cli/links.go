...
// and what stops the preference order from being re-decided, differently, in
// three places: no render site ever builds a URL.

// pullRequestRef is everything known about one pull request that could point
// somewhere. Fields may be absent, and a partially known reference still
// produces the best link available.
...
//
// GitHub first because it is where the pull request actually lives, and because
// its address is reported rather than assembled — a link that came back from
// the API cannot be wrong about the repository the way a constructed one can.
// Graphite answers only when GitHub did not.
func pullRequestURL(ref pullRequestRef) string {
	if ref.URL != "" {
		return ref.URL
	}
	return graphitePullRequest(ref)
}
