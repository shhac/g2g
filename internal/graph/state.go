...
func landedInATrunk(ctx context.Context, git Ancestry, g Graph, present map[string]bool, branch string) bool {
	for _, trunk := range g.Trunks {
		...
		// A branch the trunk shares no history with cannot be compared, which
		// is an answer rather than a failure. It gives up on the remaining
		// trunks rather than moving to the next, which is the behaviour this
		// had before the probe was shared and is left exactly as it was: it
		// looks wrong, and changing it changes what graph and prune report
		// about a branch, so it belongs to a correctness review rather than a
		// structural one.
